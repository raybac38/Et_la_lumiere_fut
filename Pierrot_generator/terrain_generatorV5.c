...
void generer_terrain(Terrain *tablo, double densite) {
    ...
    for (x0 = 0; x0 < tablo->size_max_x; x0++) {
        for (y0 = 0; y0 < tablo->size_max_y; y0++) {
            if (tablo->tab[x0][y0] == '-' || tablo->tab[x0][y0] == '|') {
                ...
                // Vérifier si le caractère est à côté d'un bord du terrain et à côté d'un '#'
                if ((x0 == 0 || x0 == tablo->size_max_x - 1 || y0 == 0 || y0 == tablo->size_max_y - 1) && ((x0 - 1 >= 0 && tablo->tab[x0 - 1][y0] == '#') || (x0 + 1 < tablo->size_max_x && tablo->tab[x0 + 1][y0] == '#') || (y0 - 1 >= 0 && tablo->tab[x0][y0 - 1] == '#') || (y0 + 1 < tablo->size_max_y && tablo->tab[x0][y0 + 1] == '#'))) {
                    continue;
                }
...
int main() {
    ...
    double densite;
    printf("Entrez la densite (entre 0 et 1) : ");
    scanf("%lf", &densite);
